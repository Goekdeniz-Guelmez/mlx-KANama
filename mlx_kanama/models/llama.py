...
class ModelArgs(BaseModelArgs):
    ...
    max_position_embeddings: Optional[int] = num_attention_heads
...
class Attention(nn.Module):
    ...
    def __call__(
        ...
        queries = mx.einsum("bld->blhd", queries.reshape(B, L, self.n_heads, self.head_dim))
        keys = mx.einsum("bld->blhd", keys.reshape(B, L, self.n_kv_heads, self.head_dim))
        values = mx.einsum("bld->blhd", values.reshape(B, L, self.n_kv_heads, self.head_dim))
        ...
        output = mx.einsum("bhld->bld", output).reshape(B, L, -1)
